...
async function proxy(request, {params}) {
  ...
  if (!['GET','HEAD'].includes(request.method) && (!origin || origin !== publicOrigin(request)))
  ...
  return admitted(request, request.method==='POST' && path.join('/')==='sessions', signal => forward(request,path,signal));
}
async function forward(request,path,signal) {
  const jar = await cookies(); let owner=jar.get('asdwise_owner')?.value;
  if (!owner || !uuid.test(owner)) owner=randomUUID();
  jar.set('asdwise_owner',owner,{httpOnly:true,sameSite:'strict',secure:publicOrigin(request).startsWith('https:'),path:'/'});
  ...
  try {
    const response=await fetch(url,{method:request.method,headers:{'x-owner':owner,'x-gateway-secret':process.env.ASDWISE_GATEWAY_SECRET,'content-type':request.headers.get('content-type')||'application/json'},body,signal,cache:'no-store'});
